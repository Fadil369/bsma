...
// .wrangler/tmp/bundle-rKkYmj/checked-fetch.js
...
// ../../../packages/shared/speech.ts
async function transcribeAudio(env, audio, opts) {
  if (!env.OPENAI_API_KEY) {
    return { text: "" };
  }
  const boundary = `----basma${crypto.randomUUID()}`;
  const formParts = [];
  const pushField = /* @__PURE__ */ __name((name, value) => {
    formParts.push(`--${boundary}\r
`);
    formParts.push(`Content-Disposition: form-data; name="${name}"\r
\r
`);
    formParts.push(`${value}\r
`);
  }, "pushField");
  const pushFile = /* @__PURE__ */ __name((name, filename, contentType, data) => {
    formParts.push(`--${boundary}\r
`);
    formParts.push(`Content-Disposition: form-data; name="${name}"; filename="${filename}"\r
`);
    formParts.push(`Content-Type: ${contentType}\r
\r
`);
    formParts.push(data);
    formParts.push(`\r
`);
  ...
  const body = new Blob(formParts, { type: `multipart/form-data; boundary=${boundary}` });
...
async function synthesizeSpeech(env, text, options) {
  if (!env.OPENAI_API_KEY) {
...
function uint8ToBlob(bytes, format) {
  const type = contentTypeForFormat(format);
  return new Blob([bytes], { type });
}
__name(uint8ToBlob, "uint8ToBlob");
...
__name(contentTypeForFormat, "contentTypeForFormat");
var src_default = app;
...
// .wrangler/tmp/bundle-rKkYmj/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-rKkYmj/middleware-loader.entry.ts
